...
export function ogmResolvers(ogm) {
  ...
  const Token = ogm.model('Singintoken')
  ...
  const resolvers = {
    Mutation: {
       ...
       */
      sendMailtoken: async (_source, {email: inputemail}) => {
        ...
        // Create account
        const createSignintoken = await Token.create({
          input: [
            {
        ...
        const [createdToken] = createSignintoken.singintokens
       ...
       * @returns {Promise<string|Error>} JWT token or Error
       */
      singInByToken: async (_source, {token}) => {
        const findSignintoken = await Token.find({
          where: {
            token
          },
          selectionSet:
        ...
        const [foundToken] = findSignintoken
